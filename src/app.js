...
function init() {
  ...
  magicWindow = new THREE.Mesh(
    new THREE.PlaneGeometry( WEBVIEW_WIDTH, WEBVIEW_HEIGHT ),
...
function loadBackground(element, customProp) {
  ...
  const texture_url = parseCssUrl(document.querySelector(element), customProp);

  // TODO: check if url is valid url before proceeding

  // create new sphere mesh, textured with URL
  const siteBackground = new THREE.Mesh(
    backgroundGeometry,
    new THREE.MeshBasicMaterial({
      shading: THREE.FlatShading,
      transparent: true,
  ...
  new TWEEN.Tween(siteBackground.material)
    .to({ opacity: 1 }, 500)
    .start();
...
function loadEnvironment_OBJ(element, customProp) {
  ...
  loaderOBJ.load( model_url, function ( obj ) {
    ...
    new TWEEN.Tween(obj.position)
      .to({x:0.15, y: -0.15, z: -0.3}, 4000)
      .start();

    new TWEEN.Tween(obj.rotation)
      .to({x:Math.PI/2, y: Math.PI/2, z: Math.PI/2}, 4000)
      .start();
  ...
  for (var i = 0; i < stereoElements.length; i++) {
    ...
    if(object.targetTransformX) {
      new TWEEN.Tween(object.threeElement.position)
        .to({x: object.targetTransformX}, 750)
    ...
    if(object.targetTransformY) {
      new TWEEN.Tween(object.threeElement.position)
        .to({y: object.targetTransformY}, 750)
        .easing(TWEEN.Easing.Quadratic.InOut)
        .start();
    }
    ...
    if(object.targetTransformZ) {
      new TWEEN.Tween(object.threeElement.position)
        .to({z: object.targetTransformZ}, 750)
        .easing(TWEEN.Easing.Quadratic.InOut)
        .start();
    }
...
window.addEventListener('keydown', function(e){
  ...
  if ((!e.metaKey) || (!e.metaKey)) {
    ...
    switch (e.keyCode) {
      ...
      case 49: { // 1 —— Load VR styles and apply to scene.
      ...
      case 50: { // 2 —— Switch to windowed mode
      ...
      case 54: { // 6 —— lean head from side to side, around window edges

        camera.updateProjectionMatrix();
        var tweenA = new TWEEN.Tween( cameraParent.rotation )
          .to({ y: 1 }, 1250 )
          .easing(TWEEN.Easing.Quadratic.Out)
  
        var tweenB = new TWEEN.Tween( cameraParent.rotation )
          .to({ y: -1 }, 3000 )
          .easing(TWEEN.Easing.Quadratic.InOut)

        var tweenC = new TWEEN.Tween( cameraParent.rotation )
          .to({ y: 0 }, 1500 )
          .easing(TWEEN.Easing.Quadratic.InOut)
